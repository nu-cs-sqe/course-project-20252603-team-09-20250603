...
|:-------------|:------------------------------------|:-----------------------------------|:--------------------------------------------------------------------------------------|:-------------|
| **TC-DC-01** | Turn of Purchase (Lower Boundary)   | `isActive = false`                 | `getIsActive()` returns `false`. `doDevCardAction()` throws `IllegalActionException`. | :x:          |
| **TC-DC-02** | End of Turn Phase Change            | Turn ends for current player.      | Card status remains `false` until the player's next turn begins.                      | :x:          |
| **TC-DC-03** | Start of Next Turn (Upper Boundary) | Turn returns to the owning player. | `isActive` is updated to `true`.                                                      | :x:          |
