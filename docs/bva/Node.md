...
- **TC1: Unoccupied node, return 0** ( :white_check_mark: )
    - **State of the system**: domain.Node.occupant == 0
    - **Expected output**: 0

- **TC2: domain.Node occupied by 1, return 1** ( :white_check_mark: )
    - **State of the system**: domain.Node.occupant == 1
    - **Expected output**: 1

### Method under test: `buildSettlement(int playerId)`
TODO: distance rule? idk if the check belongs here
#### Inputs:
- State of the node -> if domain.Node.playerId is currently 0 or not
  - Type: boolean
  - true/false
- Input player id
  - Type: cases
...
- **TC1: Successful build settlement** ( :white_check_mark: )
    - **State of the system**: domain.Node.occupant == 0, playerId == 1
    - **Expected output**: domain.Node.occupant == 1, domain.Node.infraType == "settlement"

- **TC2: Unsuccessful build settlement** ( :white_check_mark: )
    - **State of the system**: domain.Node.occupant == 1, playerId == 2
    - **Expected output**: domain.Node.occupant == 1, IllegalStateException: "Cannot settle on an already-settled node."
...
- **TC1: Successful upgrade settlement to city"** ( :white_check_mark: )
  - **State of the system**: domain.Node.occupant == 1, domain.Node.infraType == "settlement"
  - **Expected output**: domain.Node.occupant == 1, domain.Node.infraType == "city"

- **TC2: Unsuccessful upgrade city to city** ( :white_check_mark: )
  - **State of the system**: domain.Node.occupant == 1, domain.Node.infraType == "city"
  - **Expected output**: domain.Node.occupant == 1, domain.Node.infraType == "city", IllegalStateException: "Cannot upgrade a city further."

- **TC3: Unsuccessful upgrade unoccupied node to city** ( :white_check_mark: )
  - **State of the system**: domain.Node.occupant == 0, domain.Node.infraType == ""
  - **Expected output**: domain.Node.occupant == 0, domain.Node.infraType == "", IllegalStateException: "Cannot upgrade an unsettled node to city."

- **TC4: Unsuccessful upgrade city on someone else's settlement** ( :white_check_mark: )
  - **State of the system**: domain.Node.occupant == 1, domain.Node.infraType == "settlement", playerId == 2
  - **Expected output**: domain.Node.occupant == 1, domain.Node.infraType == "settlement", IllegalStateException: "Cannot build a city on an already-settled node."
